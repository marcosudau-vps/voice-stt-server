...
def _rewrite_dist_info(
    files: dict[str, bytes],
    variant: str,
    tag: tuple[str, str, str],
) -> str:
    old_dist = next(
        (n.split("/", 1)[0] for n in files if n.endswith(".dist-info/METADATA")),
        None,
    )
    if not old_dist:
    ...
    wheel_text = files[wheel_name].decode("utf-8").replace("\r\n", "\n")

    header, separator, body = wheel_text.partition("\n\n")
    if separator and body.strip():
        raise ProductWheelError("base WHEEL unexpectedly contains body content")
...
def inspect_product_wheel(path: Path) -> dict:
    with zipfile.ZipFile(path) as zf:
        ...
        wheel_bytes = zf.read(wheel_name)
        wheel_text = wheel_bytes.decode("utf-8")
        wheel_metadata = email.message_from_bytes(wheel_bytes)
    ...
    pure_value = wheel_metadata.get("Root-Is-Purelib")
    pure_line = pure_value.strip().lower() if pure_value is not None else None
