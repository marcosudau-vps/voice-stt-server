...
```bash
python -m pip install "VoiceSTT[omnilingual]"
```
...
```cmd
python api_fastapi_server\server.py --host 0.0.0.0 --port 8010 --engine kroko_onnx --model test-model-cache\kroko-onnx\Kroko-EN-Community-64-L-Streaming-001.data --realtime-engine kroko_onnx --realtime-model test-model-cache\kroko-onnx\Kroko-EN-Community-64-L-Streaming-001.data --device cpu --language en --engine-options "{\"provider\":\"cpu\",\"num_threads\":2}" --realtime-engine-options "{\"provider\":\"cpu\",\"num_threads\":1}"
```

Or use Kroko for final text with a lighter realtime engine:

```cmd
python api_fastapi_server\server.py --host 0.0.0.0 --port 8010 --engine kroko_onnx --model test-model-cache\kroko-onnx\Kroko-EN-Community-64-L-Streaming-001.data --realtime-engine whisper_cpp --realtime-model tiny.en --device cpu --language en --engine-options "{\"provider\":\"cpu\",\"num_threads\":2}"
```
...
The server passes engine names straight into VoiceSTT. Hyphenated names are
...
`VOICESTT_RUN_FASTAPI_MULTI_USER_ASR=1` runs the same test; use the `PERF`
...
`VOICESTT_FASTAPI_ASR_METRICS_JSON=/path/to/report.json` to also write the
report as JSON.

Useful overrides:

```bash
VOICESTT_FASTAPI_ASR_CLIENTS=4
VOICESTT_FASTAPI_ASR_ENGINE=sherpa_onnx_moonshine
VOICESTT_FASTAPI_ASR_MODEL=sherpa-onnx-moonshine-base-en-int8
VOICESTT_FASTAPI_ASR_REALTIME_ENGINE=sherpa_onnx_moonshine
VOICESTT_FASTAPI_ASR_REALTIME_MODEL=sherpa-onnx-moonshine-tiny-en-int8
VOICESTT_FASTAPI_ASR_DOWNLOAD_ROOT=test-model-cache/sherpa-onnx
VOICESTT_FASTAPI_ASR_DEVICE=cpu
VOICESTT_FASTAPI_ASR_MAX_WER=0.30
VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS='{"num_threads":4,"provider":"cpu"}'
VOICESTT_FASTAPI_ASR_REALTIME_ENGINE_OPTIONS='{"num_threads":2,"provider":"cpu"}'
VOICESTT_FASTAPI_ASR_REALTIME_PROCESSING_PAUSE=0.8
VOICESTT_FASTAPI_ASR_REALTIME_USE_SYLLABLE_BOUNDARIES=1
VOICESTT_FASTAPI_ASR_REALTIME_BOUNDARY_FOLLOWUP_DELAYS=0.1,0.2,0.4
```
...
```bash
VOICESTT_FASTAPI_ASR_ENGINE=kroko_onnx
VOICESTT_FASTAPI_ASR_MODEL=test-model-cache/kroko-onnx/Kroko-EN-Community-64-L-Streaming-001.data
VOICESTT_FASTAPI_ASR_REALTIME_ENGINE=kroko_onnx
VOICESTT_FASTAPI_ASR_REALTIME_MODEL=test-model-cache/kroko-onnx/Kroko-EN-Community-64-L-Streaming-001.data
VOICESTT_FASTAPI_ASR_DEVICE=cpu
VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS=provider=cpu,num_threads=2
VOICESTT_FASTAPI_ASR_REALTIME_ENGINE_OPTIONS=provider=cpu,num_threads=1
```

The engine option variables also accept `key=value` lists, for example
`VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS=num_threads=4,provider=cpu`, which is
