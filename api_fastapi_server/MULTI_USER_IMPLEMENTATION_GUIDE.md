...
`api_fastapi_server` into a multi-user realtime transcription server.
...
- `VoiceSTTService` owns one `AudioToTextRecorder`.
...
variable such as `VOICESTT_RUN_FASTAPI_MULTI_USER_ASR=1`, because it will
