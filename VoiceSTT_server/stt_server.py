...
### Available Parameters:
    ...
    - `-l, --lang, --language`: Language code for transcription; default 'de'.
...
check_and_install_packages([
    {
        'module_name': 'VoiceSTT',                 # Import module
        'attribute': 'AudioToTextRecorder',           # Specific class to check
        'install_name': 'VoiceSTT',                # Package name for pip install
    },
    {
...
from VoiceSTT.audio_recorder import DEACTIVITY_SILENCE_CONFIRMATION_DURATION
...
def parse_arguments():
    ...
    parser.add_argument('-m', '--model', type=str, default='small',
                        help='Path to the STT model or model size. Options include: tiny, tiny.en, base, base.en, small, small.en, medium, medium.en, large-v1, large-v2, or any huggingface CTranslate2 STT model such as deepdml/faster-whisper-large-v3-turbo-ct2. Default is multilingual small.')

    parser.add_argument('-r', '--rt-model', '--realtime_model_type', type=str, default='tiny',
                        help='Model size for real-time transcription. Options same as --model. This is used only if real-time transcription is enabled (enable_realtime_transcription). Default is multilingual tiny.')
    
    parser.add_argument('-l', '--lang', '--language', type=str, default='de',
                help='Language code for the STT model to transcribe in a specific language. Leave this empty for auto-detection based on input audio. Default is de. List of supported language codes: https://github.com/openai/whisper/blob/main/whisper/tokenizer.py#L11-L110')
    ...
    parser.add_argument('--silero_deactivity_detection', action='store_true', default=True,
                        help='Use the Silero model for end-of-speech detection. This can provide more robust silence detection in noisy environments. Default is True.')
    ...
    parser.add_argument('--device', type=str, default='cpu', choices=('cpu',),
                        help='CPU-only build; the only supported value is "cpu".')
...
def _recorder_thread(loop):
    ...
    print(f"{bcolors.OKGREEN}Initializing VoiceSTT server with parameters:{bcolors.ENDC}")
    for key, value in recorder_config.items():
        print(f"    {bcolors.OKBLUE}{key}{bcolors.ENDC}: {value}")
    recorder = AudioToTextRecorder(**recorder_config)
    print(f"{bcolors.OKGREEN}{bcolors.BOLD}VoiceSTT initialized{bcolors.ENDC}")
    recorder_ready.set()
