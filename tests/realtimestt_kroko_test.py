...
KROKO_REFERRAL_ENV_NAMES = (
    "VOICESTT_KROKO_ONNX_REFERRALCODE",
...
def install_repo_on_path():
    ...
    if os.path.isdir(os.path.join(repo_root, "VoiceSTT")) and repo_root not in sys.path:
...
def check_recorder_vad_runtime():
    ...
    print(
        ...
        '  python -m pip install "VoiceSTT[silero-onnx-cpu]"\n'
        "\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=("cpu",),
        default="cpu",
        help="Kroko ONNX Runtime provider. This checkout permits CPU only.",
    )
    parser.add_argument(
        "--device",
        choices=("cpu",),
        default="cpu",
        help="VoiceSTT device hint. This checkout permits CPU only.",
    )
    parser.add_argument(
        "--threads",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "VOICESTT_KROKO_ONNX_KEY/KROKO_ONNX_KEY/KROKO_KEY. "
            "Do not commit keys."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "VOICESTT_KROKO_ONNX_REFERRALCODE/KROKO_ONNX_REFERRALCODE/"
            "KROKO_REFERRALCODE."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable maximum VoiceSTT test diagnostics.",
    )
    parser.add_argument(
        "--init-only",
        action="store_true",
    ...
    import VoiceSTT.audio_recorder as audio_recorder_module

    if args.debug:
        print("Using VoiceSTT audio_recorder from %s" % audio_recorder_module.__file__)
    ...
    def configure_voicestt_live_logging(level):
        realtime_logger = logging.getLogger("voicestt")
    ...
    try:
        ...
        configure_voicestt_live_logging(recorder_config.get("level", logging.WARNING))
        if args.init_only:
