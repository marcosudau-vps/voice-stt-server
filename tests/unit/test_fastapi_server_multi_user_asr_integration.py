...
def test_native_engine_initialization_is_serialized_across_workers():
    ...
    maximum_parallel_initializers = 0

    class Engine:
        pass

    def create_engine(*_args, **_kwargs):
        nonlocal active_initializers, maximum_parallel_initializers
        with state_lock:
            active_initializers += 1
            maximum_parallel_initializers = max(
                maximum_parallel_initializers, active_initializers
            )
        time.sleep(0.05)
        with state_lock:
            active_initializers -= 1
    ...
    assert maximum_parallel_initializers == 1
...
def run_real_asr_test_enabled():
    return (
        env_flag("VOICESTT_RUN_FASTAPI_MULTI_USER_ASR")
        or env_flag("VOICESTT_RUN_FASTAPI_MULTI_USER_PERF")
    )
...
def emit_performance_report(report):
    if env_flag("VOICESTT_FASTAPI_ASR_PRINT_METRICS", default=True):
        print("\nFASTAPI_MULTI_USER_ASR_PERFORMANCE")
        print(json.dumps(report, indent=2, sort_keys=True))
    output_path = os.environ.get("VOICESTT_FASTAPI_ASR_METRICS_JSON")
    if output_path:
...
class PerformanceReportHelperTests(unittest.TestCase):
    def test_env_json_accepts_cmd_friendly_key_value_options(self):
        with mock.patch.dict(
            os.environ,
            {"VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS": "num_threads=4,provider=cpu,vad_filter=false"},
        ):
            self.assertEqual(
                env_json("VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS"),
    ...
    def test_env_json_recovers_common_cmd_quote_forms(self):
        with mock.patch.dict(
            os.environ,
            {"VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS": '{""num_threads"":4,""provider"":""cpu""}'},
        ):
            self.assertEqual(
                env_json("VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS"),
...
@unittest.skipUnless(
    ...
    "set VOICESTT_RUN_FASTAPI_MULTI_USER_ASR=1 or VOICESTT_RUN_FASTAPI_MULTI_USER_PERF=1 to run real-engine multi-user ASR load test",
)
class FastAPIMultiUserRealEngineASRTests(unittest.TestCase):
    def test_parallel_reference_wav_sessions_meet_quality_and_fairness_bounds(self):
        ...
        client_count = env_int("VOICESTT_FASTAPI_ASR_CLIENTS", 2)
        chunk_ms = env_float("VOICESTT_FASTAPI_ASR_CHUNK_MS", 32.0)
        speed = env_float("VOICESTT_FASTAPI_ASR_SPEED", 1.0)
        timeout_seconds = env_float("VOICESTT_FASTAPI_ASR_TIMEOUT", 240.0)
        max_wer = env_float("VOICESTT_FASTAPI_ASR_MAX_WER", 0.30)
        max_latency_skew_ms = env_float("VOICESTT_FASTAPI_ASR_MAX_P95_SKEW_MS", 30000.0)
        audio_seconds = samples.size / float(sample_rate)

        settings = ServerSettings(
            transcription_engine=os.environ.get("VOICESTT_FASTAPI_ASR_ENGINE", "faster_whisper"),
            realtime_transcription_engine=os.environ.get("VOICESTT_FASTAPI_ASR_REALTIME_ENGINE") or None,
            model=os.environ.get("VOICESTT_FASTAPI_ASR_MODEL", "small.en"),
            realtime_model=os.environ.get("VOICESTT_FASTAPI_ASR_REALTIME_MODEL", "tiny.en"),
            language=os.environ.get("VOICESTT_FASTAPI_ASR_LANGUAGE", "en"),
            device=os.environ.get("VOICESTT_FASTAPI_ASR_DEVICE", "cuda"),
            compute_type=os.environ.get("VOICESTT_FASTAPI_ASR_COMPUTE_TYPE", "default"),
            download_root=os.environ.get("VOICESTT_FASTAPI_ASR_DOWNLOAD_ROOT") or None,
            transcription_engine_options=env_json("VOICESTT_FASTAPI_ASR_ENGINE_OPTIONS"),
            realtime_transcription_engine_options=env_json("VOICESTT_FASTAPI_ASR_REALTIME_ENGINE_OPTIONS"),
            beam_size=env_int("VOICESTT_FASTAPI_ASR_BEAM_SIZE", 5),
            beam_size_realtime=env_int("VOICESTT_FASTAPI_ASR_BEAM_SIZE_REALTIME", 3),
            batch_size=env_int("VOICESTT_FASTAPI_ASR_BATCH_SIZE", 16),
            realtime_batch_size=env_int("VOICESTT_FASTAPI_ASR_REALTIME_BATCH_SIZE", 16),
            use_main_model_for_realtime=env_flag("VOICESTT_FASTAPI_ASR_USE_MAIN_MODEL_FOR_REALTIME"),
            max_sessions=max(client_count, 2),
            max_active_speakers=max(client_count, 2),
            max_global_inference_queue_depth=env_int(
                "VOICESTT_FASTAPI_ASR_MAX_GLOBAL_INFERENCE_QUEUE_DEPTH",
                max(64, client_count * 16),
            ),
            max_final_queue_depth_per_session=env_int(
                "VOICESTT_FASTAPI_ASR_MAX_FINAL_QUEUE_DEPTH_PER_SESSION",
                8,
            ),
            max_realtime_queue_age_ms=env_int("VOICESTT_FASTAPI_ASR_MAX_REALTIME_QUEUE_AGE_MS", 1500),
            realtime_processing_pause=env_float("VOICESTT_FASTAPI_ASR_REALTIME_PROCESSING_PAUSE", 0.25),
            realtime_min_audio_seconds=env_float("VOICESTT_FASTAPI_ASR_REALTIME_MIN_AUDIO_SECONDS", 0.25),
            post_speech_silence_duration=env_float("VOICESTT_FASTAPI_ASR_POST_SPEECH_SILENCE_DURATION", 0.6),
            min_length_of_recording=env_float("VOICESTT_FASTAPI_ASR_MIN_LENGTH_OF_RECORDING", 0.2),
            early_transcription_on_silence=env_float("VOICESTT_FASTAPI_ASR_EARLY_TRANSCRIPTION_ON_SILENCE", 0.2),
            realtime_transcription_use_syllable_boundaries=env_flag(
                "VOICESTT_FASTAPI_ASR_REALTIME_USE_SYLLABLE_BOUNDARIES"
            ),
            realtime_boundary_detector_sensitivity=env_float(
                "VOICESTT_FASTAPI_ASR_REALTIME_BOUNDARY_DETECTOR_SENSITIVITY",
                0.6,
            ),
            realtime_boundary_followup_delays=env_float_tuple(
                "VOICESTT_FASTAPI_ASR_REALTIME_BOUNDARY_FOLLOWUP_DELAYS",
                (0.05, 0.2),
            ),
        )
        manager = CollectingManager()
        service = VoiceSTTService(settings, manager)
