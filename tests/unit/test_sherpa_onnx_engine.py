...
from VoiceSTT.transcription_engines.parakeet_engine import ParakeetEngine
from VoiceSTT.transcription_engines.moonshine_engine import MoonshineEngine
from VoiceSTT.transcription_engines.sherpa_onnx_engine import (
...
class SherpaOnnxEngineTests(unittest.TestCase):
    ...
    def test_factory_creates_sherpa_engines_with_mocked_backends(self):
        ...
        for engine_name, backend_name, engine_cls in cases:
            with self.subTest(engine=engine_name):
                with patch(
                    "VoiceSTT.transcription_engines.sherpa_onnx_engine.%s"
                    % backend_name,
                    FakeSherpaBackend,
                ):
    ...
    def test_existing_engine_names_can_select_sherpa_backend(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.sherpa_onnx_engine."
            "SherpaOnnxParakeetBackend",
            FakeSherpaBackend,
        ):
    ...
    def test_missing_dependency_reports_install_hint(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.sherpa_onnx_engine.import_module",
...
class SherpaOnnxGoldenTranscriptionTests(unittest.TestCase):
    ...
    def test_transcribes_fixture_with_real_sherpa_parakeet_backend(self):
        if os.environ.get("VOICESTT_RUN_SHERPA_ONNX_PARAKEET") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_SHERPA_ONNX_PARAKEET=1 to run the "
                "sherpa-onnx Parakeet smoke test"
            )
        model_dir = Path(
            os.environ.get(
                "VOICESTT_SHERPA_ONNX_PARAKEET_MODEL",
        ...
        engine = SherpaOnnxParakeetEngine(
            TranscriptionEngineConfig(
                ...
                engine_options={
                    "num_threads": int(
                        os.environ.get("VOICESTT_SHERPA_ONNX_NUM_THREADS", "1")
                    ),
                },
            )
        )
    ...
    def test_transcribes_fixture_with_real_sherpa_moonshine_backend(self):
        if os.environ.get("VOICESTT_RUN_SHERPA_ONNX_MOONSHINE") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_SHERPA_ONNX_MOONSHINE=1 to run the "
                "sherpa-onnx Moonshine smoke test"
            )
        model_dir = Path(
            os.environ.get(
                "VOICESTT_SHERPA_ONNX_MOONSHINE_MODEL",
                "test-model-cache/sherpa-onnx/%s"
                % DEFAULT_SHERPA_ONNX_MOONSHINE_MODEL,
            )
        )
