...
class FeedAudioTests(unittest.TestCase):
    def setUp(self):
        if IMPORT_ERROR is not None:
            self.skipTest(f"VoiceSTT import failed: {IMPORT_ERROR}")
...
class GoldenTranscriptionTests(unittest.TestCase):
    def setUp(self):
        if IMPORT_ERROR is not None:
            self.skipTest(f"VoiceSTT import failed: {IMPORT_ERROR}")
        if os.environ.get("VOICESTT_RUN_GOLDEN_TRANSCRIPTION") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_GOLDEN_TRANSCRIPTION=1 to run the "
    ...
    def test_transcribes_ljspeech_sample_and_prints_result(self):
        ...
        recorder = AudioToTextRecorder(
            ...
            spinner=False,
            model=os.environ.get("VOICESTT_TEST_MODEL", "tiny"),
            language="en",
            device=os.environ.get("VOICESTT_TEST_DEVICE", "cpu"),
            compute_type=os.environ.get("VOICESTT_TEST_COMPUTE_TYPE", "int8"),
