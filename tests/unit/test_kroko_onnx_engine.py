...
from VoiceSTT.transcription_engines.kroko_onnx_engine import (
...
class KrokoOnnxEngineTests(unittest.TestCase):
    ...
    def test_factory_creates_kroko_engine_with_mocked_backend(self):
        with patch(
            "VoiceSTT.transcription_engines.kroko_onnx_engine.KrokoOnnxBackend",
            FakeKrokoBackend,
        ):
    ...
    def test_provider_mapping_from_device_and_option(self):
        ...
        with self.assertRaisesRegex(TranscriptionEngineError, "CPU-only"):
            KrokoOnnxBackend(
                TranscriptionEngineConfig(
                    model=str(model_path),
                    device="cpu",
                    engine_options={"provider": "cuda"},
                ),
                recognizer_cls=FakeKrokoRecognizer,
                numpy_module=np,
            )
...
class KrokoOnnxGoldenTranscriptionTests(unittest.TestCase):
    ...
    def test_transcribes_fixture_with_real_kroko_backend(self):
        if os.environ.get("VOICESTT_RUN_KROKO_ONNX") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_KROKO_ONNX=1 to run the Kroko-ONNX smoke test"
            )

        model_path = Path(
            os.environ.get(
                "VOICESTT_KROKO_ONNX_MODEL",
        ...
        engine_options = {
            "provider": os.environ.get("VOICESTT_KROKO_ONNX_PROVIDER", "cpu"),
            "num_threads": int(os.environ.get("VOICESTT_KROKO_ONNX_NUM_THREADS", "1")),
        }
        ...
        print("\n[VoiceSTT test] kroko_onnx expected: %s" % expected)
        print("[VoiceSTT test] kroko_onnx actual:   %s" % actual)
        print(
            "[VoiceSTT test] kroko_onnx RTF: %.3f / %.3f = %.3f"
