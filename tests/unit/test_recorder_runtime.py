...
from VoiceSTT.core.runtime import read_stdout_pipe
...
class RuntimeStdoutReaderTests(unittest.TestCase):
    def test_read_stdout_pipe_logs_received_worker_message(self):
        ...
        with mock.patch("VoiceSTT.core.runtime.time.sleep") as sleep:
            with self.assertLogs("voicestt", level="DEBUG") as logs:
                read_stdout_pipe(recorder)

        self.assertEqual(pipe.timeout, 0.1)
        sleep.assert_called_once_with(0.1)
        self.assertIn(
            "DEBUG:voicestt:Receive from stdout pipe",
            logs.output,
        )
        self.assertIn("INFO:voicestt:worker ready", logs.output)
    ...
    def test_read_stdout_pipe_logs_keyboard_interrupt_and_exits(self):
        ...
        with mock.patch("VoiceSTT.core.runtime.time.sleep") as sleep:
            with self.assertLogs("voicestt", level="INFO") as logs:
                read_stdout_pipe(recorder)
    ...
    def test_read_stdout_pipe_logs_unexpected_error_and_traceback(self):
        ...
        with mock.patch("VoiceSTT.core.runtime.time.sleep") as sleep:
            with self.assertLogs("voicestt", level="ERROR") as logs:
                read_stdout_pipe(recorder)

        sleep.assert_not_called()
        self.assertIn(
            "ERROR:voicestt:Unexpected error in read from stdout: boom",
            logs.output[0],
        )
