...
from VoiceSTT.transcription_engines.base import (
    TranscriptionEngineConfig,
    TranscriptionEngineError,
)
from VoiceSTT.transcription_engines.faster_whisper_engine import FasterWhisperEngine
...
class FasterWhisperEngineDependencyTests(unittest.TestCase):
    def test_missing_dependency_mentions_extra(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.faster_whisper_engine.import_module",
            side_effect=ModuleNotFoundError("No module named 'faster_whisper'"),
        ):
            with self.assertRaisesRegex(
                TranscriptionEngineError,
                r"VoiceSTT\[faster-whisper\]",
            ):
