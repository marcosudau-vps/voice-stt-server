...
from VoiceSTT.transcription_engines.whisper_cpp_engine import (
    PyWhisperCppBackend,
    WhisperCppEngine,
)
...
class WhisperCppFactoryTests(unittest.TestCase):
    ...
    def test_factory_creates_whisper_cpp_with_optional_backend(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.whisper_cpp_engine.PyWhisperCppBackend",
            FakeBackend,
        ):
...
class PyWhisperCppBackendTests(unittest.TestCase):
    ...
    def test_missing_pywhispercpp_reports_optional_dependency(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.whisper_cpp_engine.import_module",
...
class WhisperCppGoldenTranscriptionTests(unittest.TestCase):
    def setUp(self):
        if os.environ.get("VOICESTT_RUN_WHISPER_CPP") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_WHISPER_CPP=1 to run the slow whisper.cpp "
    ...
    def test_transcribes_ljspeech_sample_with_real_pywhispercpp_backend(self):
        ...
        engine = WhisperCppEngine(
            TranscriptionEngineConfig(
                model=os.environ.get("VOICESTT_WHISPER_CPP_MODEL", "tiny.en"),
                download_root=os.environ.get("VOICESTT_WHISPER_CPP_MODEL_DIR"),
                beam_size=int(os.environ.get("VOICESTT_WHISPER_CPP_BEAM_SIZE", "5")),
            )
        )
        result = engine.transcribe(audio, language="en")
        actual = normalize_transcript(result.text)

        print(f"\n[VoiceSTT test] whisper_cpp {sample['id']} expected: {expected}")
        print(f"[VoiceSTT test] whisper_cpp {sample['id']} actual:   {actual}")
