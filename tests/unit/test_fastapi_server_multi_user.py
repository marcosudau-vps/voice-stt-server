...
class FastAPIMultiUserSessionTests(unittest.TestCase):
    ...
    def test_empty_final_text_emits_one_discard_terminal_without_final_frame(self):
        ...
        published = session._publish_discarded_empty_final(session.generation)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with TestClient(app) as client:
                ...
                retained = [
                    ...
                    if event["event"] == "retention.test"
                ]
                first_cursor = retained[0]["cursor"]
                retained_cursor = retained[2]["cursor"]
                with events._store._lock:
                    events._store._connection.execute(
                        "DELETE FROM events WHERE cursor < ?",
                        (retained_cursor,),
                    )
                    events._store._connection.commit()
                ...
                with client.websocket_connect("/ws/logs") as logs:
                    logs.send_json({
                        ...
                        "accessToken": "test-admin-secret",
                        "channels": ["system"],
                        "afterCursor": first_cursor,
                    })
                    hello = logs.receive_json()
                    self.assertEqual(hello["oldestCursor"], retained_cursor)
                    self.assertEqual(logs.receive_json()["type"], "log.subscribed")
                    gap = logs.receive_json()
                    self.assertEqual(gap["type"], "log.gap")
                    self.assertEqual(gap["reason"], "retention")
                    self.assertEqual(gap["lostFromCursor"], first_cursor + 1)
                    self.assertEqual(gap["lostToCursor"], retained_cursor - 1)
