...
INTERNAL_RECORDER_DIR = PROJECT_ROOT / "VoiceSTT" / "core"


class AudioRecorderBoundaryTests(unittest.TestCase):
    def test_public_recorder_imports_stay_compatible(self):
        from VoiceSTT import AudioToTextRecorder as package_recorder
        from VoiceSTT.audio_recorder import (
    ...
    def test_constructor_passes_legacy_init_argument_mapping(self):
        ...
        with mock.patch(
            "VoiceSTT.audio_recorder.initialize_recorder",
    ...
    def test_audio_reader_starts_core_worker(self):
        from VoiceSTT.core import initialization
        ...
        with mock.patch(
            "VoiceSTT.core.initialization.start_recorder_worker",
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name == "VoiceSTT.audio_recorder":
                            offenders.append(path.name)
                elif isinstance(node, ast.ImportFrom):
                    if node.module == "VoiceSTT.audio_recorder":
                        offenders.append(path.name)
                    elif node.module == "VoiceSTT" and any(
