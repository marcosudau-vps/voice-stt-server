...
from VoiceSTT.transcription_engines.openai_whisper_engine import (
...
class OpenAIWhisperFactoryTests(unittest.TestCase):
    ...
    def test_factory_creates_openai_whisper_with_optional_backend(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.openai_whisper_engine.OpenAIWhisperBackend",
            FakeBackend,
        ):
...
class OpenAIWhisperBackendTests(unittest.TestCase):
    ...
    def test_missing_openai_whisper_reports_optional_dependency(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.openai_whisper_engine.import_module",
...
class OpenAIWhisperGoldenTranscriptionTests(unittest.TestCase):
    def setUp(self):
        if os.environ.get("VOICESTT_RUN_OPENAI_WHISPER") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_OPENAI_WHISPER=1 to run the slow "
    ...
    def test_transcribes_ljspeech_sample_with_real_openai_whisper_backend(self):
        ...
        engine = OpenAIWhisperEngine(
            TranscriptionEngineConfig(
                model=os.environ.get("VOICESTT_OPENAI_WHISPER_MODEL", "tiny.en"),
                download_root=os.environ.get("VOICESTT_OPENAI_WHISPER_MODEL_DIR"),
                device=os.environ.get("VOICESTT_OPENAI_WHISPER_DEVICE", "cpu"),
                compute_type=os.environ.get("VOICESTT_OPENAI_WHISPER_COMPUTE_TYPE", "float32"),
            )
        )
        result = engine.transcribe(audio, language="en")
        actual = normalize_transcript(result.text)

        print("\n[VoiceSTT test] openai_whisper expected: %s" % expected)
        print("[VoiceSTT test] openai_whisper actual:   %s" % actual)
