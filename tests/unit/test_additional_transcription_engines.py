...
from VoiceSTT.transcription_engines.cohere_transcribe_engine import (
    CohereTranscribeBackend,
    CohereTranscribeEngine,
)
from VoiceSTT.transcription_engines.granite_speech_engine import (
    GraniteSpeechBackend,
    GraniteSpeechEngine,
)
from VoiceSTT.transcription_engines.moonshine_engine import (
    MoonshineBackend,
    MoonshineEngine,
)
from VoiceSTT.transcription_engines.parakeet_engine import (
    ParakeetEngine,
    ParakeetNeMoBackend,
)
from VoiceSTT.transcription_engines.qwen3_asr_engine import (
    Qwen3ASRBackend,
    Qwen3ASREngine,
)
...
class AdditionalEngineFactoryTests(unittest.TestCase):
    ...
    def _backend_patch_target(engine_name, backend_name):
        ...
        return "VoiceSTT.transcription_engines.%s.%s" % (module, backend_name)
...
class ParakeetEngineTests(unittest.TestCase):
    ...
    def test_missing_nemo_reports_wsl2_hint(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.parakeet_engine.import_module",
...
class CohereTranscribeEngineTests(unittest.TestCase):
    ...
    def test_missing_transformers_reports_optional_dependency(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.hf_transformers_engines.import_module",
...
class Qwen3ASREngineTests(unittest.TestCase):
    ...
    def test_missing_qwen_asr_reports_optional_dependency(self):
        ...
        with patch(
            "VoiceSTT.transcription_engines.qwen3_asr_engine.import_module",
...
class AdditionalEngineGoldenTranscriptionTests(unittest.TestCase):
    ...
    def assert_engine_transcribes_fixture(self, engine, language="en"):
        ...
        print("\n[VoiceSTT test] %s expected: %s" % (engine.engine_name, expected))
        print("[VoiceSTT test] %s actual:   %s" % (engine.engine_name, actual))

        self.assertTrue(actual)
        self.assertIn(" ".join(expected.split()[:2]), actual)

    def test_transcribes_fixture_with_real_parakeet_backend(self):
        if os.environ.get("VOICESTT_RUN_PARAKEET") != "1":
            self.skipTest("Set VOICESTT_RUN_PARAKEET=1 to run the Parakeet smoke test")
        if os.name == "nt":
        ...
        engine = ParakeetEngine(
            TranscriptionEngineConfig(
                model=os.environ.get(
                    "VOICESTT_PARAKEET_MODEL",
                    "nvidia/parakeet-tdt-0.6b-v3",
                ),
                device=os.environ.get("VOICESTT_PARAKEET_DEVICE", "cuda"),
    ...
    def test_transcribes_fixture_with_real_cohere_backend(self):
        if os.environ.get("VOICESTT_RUN_COHERE_TRANSCRIBE") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_COHERE_TRANSCRIBE=1 to run the Cohere smoke test"
            )

        engine = CohereTranscribeEngine(
            TranscriptionEngineConfig(
                model=os.environ.get(
                    "VOICESTT_COHERE_TRANSCRIBE_MODEL",
                    "CohereLabs/cohere-transcribe-03-2026",
                ),
                device=os.environ.get("VOICESTT_COHERE_TRANSCRIBE_DEVICE", "cpu"),
                compute_type=os.environ.get("VOICESTT_COHERE_TRANSCRIBE_COMPUTE_TYPE", "default"),
                download_root=os.environ.get("VOICESTT_HF_MODEL_DIR"),
    ...
    def test_transcribes_fixture_with_real_granite_backend(self):
        if os.environ.get("VOICESTT_RUN_GRANITE_SPEECH") != "1":
            self.skipTest(
                "Set VOICESTT_RUN_GRANITE_SPEECH=1 to run the Granite smoke test"
            )

        engine = GraniteSpeechEngine(
            TranscriptionEngineConfig(
                model=os.environ.get(
                    "VOICESTT_GRANITE_SPEECH_MODEL",
                    "ibm-granite/granite-speech-4.1-2b",
                ),
                device=os.environ.get("VOICESTT_GRANITE_SPEECH_DEVICE", "cpu"),
                compute_type=os.environ.get("VOICESTT_GRANITE_SPEECH_COMPUTE_TYPE", "float32"),
                download_root=os.environ.get("VOICESTT_HF_MODEL_DIR"),
            )
        )
        self.assert_engine_transcribes_fixture(engine, language="en")
    ...
    def test_transcribes_fixture_with_real_qwen3_asr_backend(self):
        if os.environ.get("VOICESTT_RUN_QWEN3_ASR") != "1":
            self.skipTest("Set VOICESTT_RUN_QWEN3_ASR=1 to run the Qwen3-ASR smoke test")
        backend = os.environ.get("VOICESTT_QWEN3_ASR_BACKEND", "transformers")
        if backend == "vllm" and os.name == "nt":
            self.skipTest("Run the Qwen3-ASR vLLM smoke test from Linux or WSL2")

        engine = Qwen3ASREngine(
            TranscriptionEngineConfig(
                model=os.environ.get("VOICESTT_QWEN3_ASR_MODEL", "Qwen/Qwen3-ASR-1.7B"),
                device=os.environ.get("VOICESTT_QWEN3_ASR_DEVICE", "cpu"),
                compute_type=os.environ.get("VOICESTT_QWEN3_ASR_COMPUTE_TYPE", "float32"),
                download_root=os.environ.get("VOICESTT_HF_MODEL_DIR"),
    ...
    def test_transcribes_fixture_with_real_moonshine_backend(self):
        if os.environ.get("VOICESTT_RUN_MOONSHINE") != "1":
            self.skipTest("Set VOICESTT_RUN_MOONSHINE=1 to run the Moonshine smoke test")

        engine = MoonshineEngine(
            TranscriptionEngineConfig(
                model=os.environ.get(
                    "VOICESTT_MOONSHINE_MODEL",
                    "UsefulSensors/moonshine-streaming-medium",
                ),
                device=os.environ.get("VOICESTT_MOONSHINE_DEVICE", "cpu"),
                compute_type=os.environ.get("VOICESTT_MOONSHINE_COMPUTE_TYPE", "float32"),
                download_root=os.environ.get("VOICESTT_HF_MODEL_DIR"),
            )
        )
        self.assert_engine_transcribes_fixture(engine, language="en")
