...
1. Generate the expected utterance JSON with a small CPU/int8 model.
2. Run the same file again with the target CPU/int8 configuration.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate CPU reference utterances and compare CPU output."
    )
    ...
    parser.add_argument("--reference-device", type=str, default="cpu", choices=("cpu",))
    parser.add_argument("--reference-compute-type", type=str, default="int8")
...
def main():
    ...
    if args.mode in ("generate", "both"):
        reference_result = run_streaming_text_pass(
            "Reference expected pass",
            args,
            audio,
            args.reference_device,
            args.reference_compute_type,
        )
        write_expected_json(args.expected_json, args, audio, reference_result)
