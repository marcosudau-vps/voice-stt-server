...
def create_silero_vad_model(
    ...
    """
    log = logger or logging.getLogger("voicestt")
    ...
    if requested == SILERO_BACKEND_PYTORCH_CUDA:
        raise SileroVadError(
            "This project build is CPU-only; use a Silero ONNX or PyTorch CPU backend."
        )
...
def _create_pytorch_vad(device):
    ...
    if device != "cpu":
        raise SileroVadError("This project build is CPU-only.")
    try:
