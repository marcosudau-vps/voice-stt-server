...
def _log_extended_initialization(use_extended_logging, init_args):
    ...
    if use_extended_logging:
        logger.info("VoiceSTT was called with these parameters:")
...
def _initialize_transcription_runtime(recorder, recorder_cls):
    ...
    if str(recorder.device).lower() != "cpu":
        logger.warning(
            "Ignoring requested device %r: this project build is CPU-only and "
            "does not probe the GPU driver.",
            recorder.device,
        )
    recorder.device = "cpu"
    recorder.gpu_device_index = 0
...
def _finish_initialization(recorder):
    ...
    logger.debug('VoiceSTT initialization completed successfully')
