...
def _provider_from_config(config, options):
    ...
    if explicit and str(explicit).lower() != "cpu":
        raise TranscriptionEngineError(
            "This project build is CPU-only; kroko-onnx provider must be 'cpu'."
        )
    return "cpu"
...
class KrokoOnnxBackend:
    ...
    def _resolve_model_path(self):
        ...
        resolved_local = resolve_kroko_model(
            self.config.model or DEFAULT_KROKO_ONNX_MODEL,
            self.config.download_root,
            self.engine_options,
        )
        if Path(resolved_local).is_file():
            return Path(resolved_local)
