...
`ASRInferencePipeline`. The adapter is lazy-loaded, so normal VoiceSTT
...
If `model` is left at VoiceSTT's default Whisper value, the adapter selects
...
ships the v2 cards. The VoiceSTT extra requires `omnilingual-asr>=0.2.0`
...
VoiceSTT default because it did not pass validation for this integration.
...
```bash
curl -L https://raw.githubusercontent.com/KoljaB/VoiceSTT/release/v1.0.1/tests/voicestt_omnilingual_test.py -o voicestt_omnilingual_test.py
python voicestt_omnilingual_test.py --file-smoke --device cuda --cache-home .home-omnilingual-smoke
```
...
```bash
python voicestt_omnilingual_test.py --microphone --device cuda
```
...
locations. VoiceSTT does not move or delete those files.
...
| `transcription_engine_options["batch_size"]` | Overrides the VoiceSTT `batch_size` value. |
...
Omnilingual. The `api_fastapi_server` reference app is not installed by the
...
This recipe targets `api_fastapi_server/server.py`, not the installed
...
```bash
curl -L https://raw.githubusercontent.com/KoljaB/VoiceSTT/release/v1.0.1/tests/voicestt_omnilingual_test.py -o voicestt_omnilingual_test.py
python voicestt_omnilingual_test.py --file-smoke --device cuda
```
