...
VoiceSTT's primary public API is the local recorder:
...
from VoiceSTT import AudioToTextRecorderClient
```
...
engine contract in `VoiceSTT/transcription_engines/base.py`.
...
- Does `from VoiceSTT import AudioToTextRecorder` still lazy-load?
- Does `from VoiceSTT.audio_recorder import AudioToTextRecorder` still work?
