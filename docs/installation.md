...
```bash
python -m pip install "VoiceSTT[faster-whisper]"
```

Core package only, without a transcription engine or wake-word backend:

```bash
python -m pip install VoiceSTT
```
...
| --- | --- |
| Core VoiceSTT | Python 3.11 or newer. |
...
```bash
python -m pip install "VoiceSTT[faster-whisper,porcupine]"
python -m pip install "VoiceSTT[whisper-cpp,openwakeword]"
```
...
| `kroko-builder` | VoiceSTT builder helper, `huggingface_hub` | Builds/installs Kroko-ONNX from upstream and downloads public Community models. On Windows, the builder currently requires CPython 3.12 x64. For `AudioToTextRecorder` smoke tests or live microphone use, combine it with `silero-onnx-cpu`. |
...
VoiceSTT uses Porcupine for backward compatibility, so install the
`porcupine` extra.
...
VoiceSTT can run on CPU with small models, but CUDA is strongly preferred
...
before installing VoiceSTT, for example:
...
| --- | --- | --- |
| `faster_whisper` | `python -m pip install "VoiceSTT[faster-whisper]"` | Downloads CTranslate2 models automatically through faster-whisper. |
| `whisper_cpp` | `python -m pip install "VoiceSTT[whisper-cpp]"` | `pywhispercpp` can download known ggml models; local paths are also supported. |
| `openai_whisper` | `python -m pip install "VoiceSTT[openai-whisper]"` | Downloads OpenAI Whisper models automatically to its cache or `download_root`. |
...
```bash
python -m pip install "VoiceSTT[wakewords]"
```
