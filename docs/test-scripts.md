...
| `tests/voicestt_test.py` | Full interactive demo | Rich console demo with realtime transcription, final text, and optional keyboard typing. |
...
| Script | Category | Purpose |
| --- | --- | --- |
| `tests/advanced_talk.py` | Assistant demo | Combines VoiceSTT with RealtimeTTS and LLM calls. Requires API keys and TTS dependencies. |
...
| Script | Category | Purpose |
| --- | --- | --- |
| `tests/voicestt_speechendpoint.py` | Legacy/manual experiment | Experiments with speech endpoint detection and display behavior. |
| `tests/voicestt_speechendpoint_binary_classified.py` | Legacy/manual experiment | Variant of the speech endpoint experiment with binary classification logic. |
| `tests/voicestt_test_hotkeys_v2.py` | Manual UI/hotkey demo | Rich console and hotkey workflow for dictation-style usage. |
| `tests/voicestt_test_stereomix.py` | Manual audio-device demo | Tests stereo mix/system audio input behavior. |
...
```bash
python -m pip install "VoiceSTT[whisper-cpp]" rich pyautogui colorama
python tests/voicestt_test_whispercpp.py --profile balanced
```
