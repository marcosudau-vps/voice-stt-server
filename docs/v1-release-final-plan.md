...
[v1-preservation-release.md](v1-preservation-release.md). Die fünf am Ende
...
2. Server-Source auf `release/v1.0.0-prep` fokussiert committen und diesen
   Branch normal pushen. Die Push-Trigger von
   `v1-release-prep-ci.yml` und `v1-release-build-validation.yml`
   müssen den Branch einschließen. Ihre Runs sind **keine** Candidate- oder
   Publish-Runs.
3. Beide Server-CI-Runs auf exakt diesem Commit/Tree abwarten. Die
