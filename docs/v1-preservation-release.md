...
Pushes to `release/v1.0.0-prep` and to the new
`review/v1-release-prep-final-clean*` line run real build validation.
The final clean commit is recreated directly from historical V1 `main`;
the 18-commit preparation branch is **not** fast-forwarded to `main`.
...
Publish run. Existing `main`, dirty review, earlier clean review, V2 branches,
