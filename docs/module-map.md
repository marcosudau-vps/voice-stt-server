...
| `VoiceSTT/core/openwakeword_catalog.py` | Authoritative wake-word model catalog and resolution authority: casefold lookups, explicit aliases, collision checking, versioned public catalog without path leaks, and disable provider hook. | Optional dependency model root discovery and models.json parsing. | `tests/unit/test_wakeword_contract.py`, `tests/unit/test_wakeword.py`. |
| `VoiceSTT/core/wakeword.py` | Wake-word engine initialization and runtime candidate processing: selected-only model loading, sensitivity validation [0.0 - 1.0], deterministic highest score and canonical ID alphabetical tie-break. | Model worker loading, audio chunk scoring. | `tests/unit/test_wakeword_contract.py`, `tests/unit/test_wakeword.py`, `tests/unit/test_wakeword_evidence_harness.py`. |
...
| `api_fastapi_server/wake_admission.py` | Server-authoritative `WakeAdmissionCoordinator` managing domain latch lifecycle and activation admission with `ActivationController`. | Latch release bound to canonical SRV-030 safe input close; validate with `tests/unit/test_wakeword_contract.py`. |
