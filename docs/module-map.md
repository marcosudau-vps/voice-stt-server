...
| `api_fastapi_server/settings_control/*` | AP-SRV-050 settings control plane: registry (single source of key metadata), value validation, applied policy semantics, monotonic `settingsRevision`, atomic server-default patches, `session_settings.patch` port and the `ActivationSettingsProvider` seam. | Owns the frozen `activation.*Ms` key definitions and the server-default overlay. Reuses `RuntimeConfigStore`/`ServerSettings` mechanisms through narrow adapters; must stay free of torch/VoiceSTT recorder imports so it stays unit-testable. The FastAPI router imports fastapi lazily. |
...
| Settings control plane | `tests/unit/test_settings_control_plane.py`, `tests/unit/test_settings_control_rest.py` | Registry/schema contract, timing boundaries, watchdog cross-field rule, revision concurrency/atomicity, apply policies, session patch port, secret redaction and the v2 REST surface. |
