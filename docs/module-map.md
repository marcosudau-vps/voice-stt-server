...
| `VoiceSTT/core/wakeword_catalog.py` | AP-SRV-060 catalog authority: the bundled `models.json` as the one manifest authority, the canonical-only wire admission (`admit_selection`) next to the tolerant config resolver (`resolve_human_selection`), the memoised loadability probe of the selected artifacts, the immutable `WakeWordCatalogSnapshot` with public and internal projections, availability incl. the global disable projection, and the thread-safe `WakeWordCatalogAuthority` with last-known-good swap and `catalogRevision`. | Exactly one server-wide instance. Filesystem discovery is diagnostics only; refresh and global disable are linearised in one locked operation and every result carries the snapshot it describes. Public payloads never carry paths, and every entry carries its own revision. |
| `VoiceSTT/core/wake_detection.py` | Detection domain: `RawWakeCandidate` (diagnostics) versus `AcceptedWakeDetection` (the only published wake fact), the immutable `WakeRuntimePolicy` that binds the `next_activation` wake settings, the stable multi-model rule, the measured receptive-field arithmetic and the `WakeDetectionEvaluator` (threshold, latch, de-duplication, configured cooldown). | Detector hygiene only. It knows nothing about activations and holds no second state machine, and it reads the settings authority rather than copying it. The implicit de-duplication window dies at the safe input close; only a configured cooldown survives it. |
| `VoiceSTT/core/wake_audio_boundary.py` | The wake/user-speech audio boundary: measured detection sample and receptive field versus the *estimated* wake end, the pre-roll and the released transcript audio. | Replaces the blanket fixed-duration cut on the v2 path. `0 ms` pre-roll must stay correct. No name or projection may claim a proven acoustic wake end while WW-19 is unmeasured - `boundaryBasis`/`boundaryMeasured` say so explicitly. |
| `api_fastapi_server/wake_admission.py` | The seam between detection and activation: raw candidate to detection evaluation to `ActivationController.activate(source="wake_word")`, and only on success the latch, the accepted `activationId` and exactly one `wakeword.detected`. Owns the explicit commit boundary (`WakeActivationOutcome`) and the commit probe. | The wake latch lives here, never in the `ActivationController`. A refused admission leaves no event, no latch and no second activation; a failure *after* the commit is never turned back into a refusal. |
