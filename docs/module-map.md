...
| `api_fastapi_server/protocol_v2/session.py` | `ProtocolSessionState`: `stateVersion`, `eventSeq`/`eventId` and the registry that makes a transport retry re-send one logical event. | Owns wire versioning only - never a phase, a deadline, a trigger lock or ledger state. |
...
| `api_fastapi_server/protocol_v2/ports.py` | Narrow settings and wake-word ports (`REQUIRES_AP_SRV_050_BINDING`, `REQUIRES_AP_SRV_060_BINDING`). | Read/validate only. Neither port may become a second registry or a second wake engine. |
...
| Protocol v2 wire contract | `tests/unit/test_protocol_v2_contract.py`, `tests/unit/test_protocol_v2_e2e.py`, `tests/unit/test_protocol_v2_races.py` | Frozen contract vectors from `tests/contracts/protocol-v2-vectors.json`, handshake refusals and close codes, strict envelope, replay/conflict, `eventId`/`eventSeq`/`stateVersion`, exactly-once `activation.input_closed`, snapshot combinations and repeated ordering proofs. |
