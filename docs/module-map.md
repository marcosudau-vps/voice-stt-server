...
| `api_fastapi_server/protocol_v2/ports.py` | Settings and wake-word adapters: the settings port proxies the AP-SRV-050 session settings control (revision, effective settings, patch result); the wake-word port proxies the AP-SRV-060 catalog authority (capabilities, `catalogRevision`, atomic selection admission). | Adapter only. No value storage, no second registry, no second resolver and no second wake engine. |
| `VoiceSTT/core/wakeword_catalog.py` | AP-SRV-060 catalog authority: the bundled `models.json` as the one manifest authority, the canonical-only wire admission (`admit_selection`) next to the tolerant config resolver (`resolve_human_selection`), per-backend health resolved at load and at every refresh (schema, ids/aliases, declared integrity, both declared formats, runtime availability, pipeline assets, real probe load), the immutable `WakeWordCatalogSnapshot` with public and internal projections, availability incl. the global disable projection, the common-backend gate of one selection, and the thread-safe `WakeWordCatalogAuthority` with last-known-good swap and `catalogRevision`. | Exactly one server-wide instance. Filesystem discovery is diagnostics only; refresh and global disable are linearised in one locked operation and every result carries the snapshot it describes. A missing runtime is an unhealthy backend, never a passed probe. Unloadable entries stay publicly listed with `available=false` and a machine-readable reason; public payloads never carry paths, and every entry carries its own revision. |
| `VoiceSTT/core/wake_detection.py` | Detection domain: the `WakeHitTracker` (score threshold, minimum consecutive prediction frames, contiguous hit regions, qualification, trailing-edge finalization, first-finalized-wins and the deterministic tie-breakers), the immutable `WakeAttemptPolicy` that freezes one attempt onto one `settingsRevision`, `WakeHit`/`AcceptedWakeDetection` as the domain facts, `RawWakeCandidate` as diagnostics, and the `WakeDetectionEvaluator` (latch and configured operator cooldown). | Our domain logic only - feature extraction, loaders, VAD, noise suppression and the prediction stay upstream. It knows nothing about activations and holds no second state machine, and it reads the settings authority rather than copying it. One attempt never mixes two settings revisions. |
| `VoiceSTT/core/wake_audio_boundary.py` | The wake/user-speech audio boundary: the operational zero point (the trailing edge of the winning qualified hit region), the pre-roll clamped against the retained audio history, and the released transcript audio. | Replaces the blanket fixed-duration cut on the v2 path. `0 ms` pre-roll releases exactly at the zero point. The zero point is a defined product boundary (`boundaryBasis`/`boundaryDefined`), not an estimate; what stays open is the empirical calibration (WW-18/WW-19). |
| `VoiceSTT/core/openwakeword_engine.py` | The thin OpenWakeWord adapter: one upstream `openwakeword.Model` per live session, one common inference backend, selected models only, the prediction-frame accounting that ignores repeated/cached scores, the detector-only gain on a PCM copy, and the optional Speex/VAD wiring. | No fork and no copy of feature extraction, loaders, Speex, Silero VAD or the prediction. A recorder chunk is never a prediction frame, and the original PCM is never modified. |
| `VoiceSTT/core/wake_backend.py` | The dual-backend policy: preference order per platform (`auto` prefers ONNX on Windows and TFLite/LiteRT on Linux), the common fallback for the *whole* selection and the machine-readable rejection reasons `backend_unavailable` / `no_common_backend`. | Owns the choice, never the health - health is the catalog's answer. An explicitly requested backend never switches silently, and there is no per-model backend mixture inside one live engine. |
| `api_fastapi_server/wake_admission.py` | The seam between detection and activation: finalized hit to detection evaluation to `ActivationController.activate(source="wake_word")`, and only on success the latch, the accepted `activationId` and the exactly-once mint of one logical `wakeword.detected` through the `LogicalWakeEventLedger`. Owns the explicit commit boundary (`WakeActivationOutcome`), the commit probe and the retry of an already minted event. | The wake latch lives here, never in the `ActivationController`. A refused admission leaves no event, no latch and no second activation; a failure *after* the commit is never turned back into a refusal. Logical minting and fallible transport delivery are separate, so a transport failure can neither lose the one event nor duplicate it on a retry. |
...
| Wake-word catalog, detection and audio boundary | `tests/unit/test_wakeword_catalog.py`, `tests/unit/test_wakeword_catalog_api.py`, `tests/unit/test_wake_detection.py`, `tests/unit/test_wake_audio_boundary.py`, `tests/unit/test_wake_admission.py`, `tests/unit/test_wakeword_selected_only.py`, `tests/unit/test_wakeword_recording_path.py`, `tests/unit/test_wakeword_session_e2e.py`, `tests/unit/test_wakeword_root_findings_c2.py`, `tests/unit/test_wakeword_root_findings_c3.py` | Manifest schema and id/alias collisions, the one resolver, availability and global disable, refresh with last-known-good, the `GET /api/v2/wake-words` contract and the admin refresh, atomic session admission, the real selected-only loader arguments, exactly-once `wakeword.detected`, the latch until safe input close, FIND-011 duplicate suppression and the pre-roll/wake boundary. |
