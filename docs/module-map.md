...
| `VoiceSTT/core/wakeword_catalog.py` | AP-SRV-060 catalog authority: the bundled `models.json` as the one manifest authority, the single tolerant resolver (`normalize_wake_word_token`), the immutable `WakeWordCatalogSnapshot` with public and internal projections, availability incl. the global disable projection, and the thread-safe `WakeWordCatalogAuthority` with last-known-good swap and `catalogRevision`. | Exactly one server-wide instance. Filesystem discovery is diagnostics only; a candidate is swapped in atomically or not at all. Public payloads never carry paths. |
| `VoiceSTT/core/wake_detection.py` | Detection domain: `RawWakeCandidate` (diagnostics) versus `AcceptedWakeDetection` (the only published wake fact), the stable multi-model rule, the measured receptive-field arithmetic and the `WakeDetectionEvaluator` (threshold, latch, re-arm). | Detector hygiene only. It knows nothing about activations and holds no second state machine. |
| `VoiceSTT/core/wake_audio_boundary.py` | The wake/user-speech audio boundary: detector history, wake-word audio, wake end, pre-roll and released transcript audio, anchored to the real sample position of the accepted detection. | Replaces the blanket fixed-duration cut on the v2 path. `0 ms` pre-roll must stay correct. |
| `api_fastapi_server/wake_admission.py` | The seam between detection and activation: raw candidate to detection evaluation to `ActivationController.activate(source="wake_word")`, and only on success the latch, the accepted `activationId` and exactly one `wakeword.detected`. | The wake latch lives here, never in the `ActivationController`. A refused admission leaves no event, no latch and no second activation. |
